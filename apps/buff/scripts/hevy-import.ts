...
const BARBELL = new Set<ExerciseId>([
  "bench-press",
  "squat",
  "barbell-row",
  "overhead-press",
...
function buildExercises(items: DayItem[], T: (key: string) => HevyTemplate): HevyExercise[] {
  ...
  for (const item of items) {
    if (item.ex) {
      ...
      const notes =
        [item.star ? "★ weak-point priority" : null, item.hint ?? null]
          .filter(Boolean)
          .join(" · ") || null
      out.push({
    ...
    if (name.startsWith("Superset")) {
      ...
      out.push(
        {
          exercise_template_id: T("cable-curl").id,
          superset_id: sid,
          rest_seconds: 60,
          notes: "superset with pushdowns",
          sets: mkSets(3, 12, null, 7)
        },
        {
          exercise_template_id: T("triceps-pushdown").id,
          superset_id: sid,
          rest_seconds: 60,
          notes: "superset with curls",
          sets: mkSets(3, 12, null, 9)
        }
      )
      ...
      out.push(
        {
          exercise_template_id: T("db-curl").id,
          superset_id: sid,
          rest_seconds: 60,
          notes: "giant set 1/3",
          sets: mkSets(3, 10, 12, 7)
        },
        {
          exercise_template_id: T("hammer-curl").id,
          superset_id: sid,
          rest_seconds: 60,
          notes: "giant set 2/3",
          sets: mkSets(3, 10, 12, 7)
        ...
        {
          exercise_template_id: T("overhead-triceps").id,
          superset_id: sid,
          rest_seconds: 60,
          notes: "giant set 3/3",
          sets: mkSets(3, 10, 12, 7)
      ...
      out.push(
        {
          exercise_template_id: T("knee-raise").id,
          superset_id: null,
          rest_seconds: 60,
          notes: "on the dip tower",
          sets: mkSets(3, 10, 15, null)
        },
        {
          exercise_template_id: T("cable-crunch").id,
          superset_id: null,
          rest_seconds: 60,
          notes: null,
          sets: mkSets(3, 10, 15, null)
...
for (const r of routines) {
  ...
  for (const e of r.exercises) {
    ...
    console.log(
      `   ${e.sets.length} × ${reps}${w} · rest ${e.rest_seconds}s · ${t?.title}${ss}${e.notes ? ` — ${e.notes}` : ""}`
    )
  }
}
...
if (existing) {
  ...
  const created = await hevy("/routine_folders", {
    method: "POST",
    body: JSON.stringify({ routine_folder: { title: "buff" } })
  })
