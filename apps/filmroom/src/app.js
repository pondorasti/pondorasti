...
;(() => {
  ...
  class View {
    constructor(data, index) {
      ...
      this.label.className = "viewport-label"
      const small = document.createElement("small")
      small.textContent = String(index + 1).padStart(2, "0")
      const name = document.createElement("span")
      name.textContent = data.name
      this.label.append(small, name)
      this.side = document.createElement("span")
      this.side.className = "corner-tag"
      this.side.textContent = data.laterality
      this.stats = document.createElement("span")
      this.stats.className = "viewport-stats"
      this.orientation = document.createElement("span")
      this.orientation.className = "orientation"
    ...
    zoom(factor, x, y) {
      ...
      const cx = this.panel.clientWidth / 2,
        cy = this.panel.clientHeight / 2
      x = x ?? cx
      y = y ?? cy
    ...
    bindGestures() {
      ...
      const end = (event) => {
        ...
        if (!pointers.size) this.panel.classList.remove("dragging")
      }
      ...
      this.panel.addEventListener(
        ...
        (event) => {
          ...
          const delta =
            event.deltaY *
            (event.deltaMode === 1 ? 16 : event.deltaMode === 2 ? this.panel.clientHeight : 1)
          this.zoom(
  ...
  function syncControls() {
    ...
    $("center").min = view.data.centerMin
    $("center").max = view.data.centerMax
    $("center").step = "any"
    $("width").step = "any"
    $("width").max = view.data.widthMax
    $("resolution").textContent = `${view.data.columns} × ${view.data.rows}`
    $("bit-depth").textContent = `${view.data.bits}-bit DICOM`
    $("center").value = Math.round(view.center)
    $("width").value = Math.round(view.width)
    ...
    $("center-value").textContent = Math.round(view.center)
    $("width-value").textContent = Math.round(view.width)
    setPressed("invert", view.invert)
    setPressed("flip", view.flip)
    const close = (a, b) => Math.abs(a - b) < 1
    setPressed(
      "original-preset",
      close(view.center, view.data.center) && close(view.width, view.data.width)
    )
    setPressed(
      "auto-preset",
      close(view.center, view.data.autoCenter) && close(view.width, view.data.autoWidth)
    )
  ...
  function setLayout(value) {
    ...
    setPressed("single-layout", !compare)
    setPressed("compare-layout", compare)
    views.forEach((view) => {
      view.fitted = true
    })
    select(active)
  ...
  function setTool(value) {
    tool = value
    setPressed("pan-tool", tool === "pan")
    setPressed("window-tool", tool === "window")
  ...
  function showStudy(nextStudy) {
    ...
    setPressed("single-layout", true)
    setPressed("compare-layout", false)
    ...
    study.images.forEach((data, index) => {
      const view = new View(data, index)
      views.push(view)
      ...
      meta.textContent = `${data.columns} × ${data.rows}`
  ...
  $("center").addEventListener("input", (event) =>
    views[active].setWindow(Number(event.target.value), views[active].width)
  )
  $("width").addEventListener("input", (event) =>
    views[active].setWindow(views[active].center, Number(event.target.value))
  )
  $("original-preset").addEventListener("click", () => {
    const view = views[active]
    view.setWindow(view.data.center, view.data.width)
  })
  $("auto-preset").addEventListener("click", () => {
    const view = views[active]
    view.setWindow(view.data.autoCenter, view.data.autoWidth)
  })
  ...
  for (const [button, panel, className] of [
    ...
    $(button).addEventListener("click", () => {
      ...
      views.forEach((view) => {
        if (view.fitted) view.fit()
        view.requestDraw()
      })
    })
  }
  $("zoom-in").addEventListener("click", () => views[active].zoom(1.25))
  $("zoom-out").addEventListener("click", () => views[active].zoom(0.8))
  ...
  document.addEventListener("keydown", (event) => {
    ...
    const actions = {
      ...
      i: () => $("invert").click(),
      c: () => setLayout(!compare),
      "+": () => views[active].zoom(1.25),
      "=": () => views[active].zoom(1.25),
      "-": () => views[active].zoom(0.8),
  ...
  function setLoadedControls(enabled) {
    document
      .querySelectorAll(
        ".toolbar button, .display-settings button, .display-settings input, #export-png"
      )
  ...
  function openBundledStudy() {
    ...
    studyWorker.onmessage = ({ data }) => {
      if (data.type === "progress") $("load-message").textContent = data.message
        ...
        finishLoading()
        showStudy(data.study)
      }
    }
    studyWorker.onerror = () =>
      showLoadError("The image decoder could not start. Please try again.")
