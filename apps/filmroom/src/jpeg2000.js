...
export function validateHeader(encoded, expected) {
  ...
  const matches = data.getUint16(0) === 0xff4f && data.getUint16(2) === 0xff51 &&
    data.getUint32(8) - data.getUint32(16) === expected.columns &&
    data.getUint32(12) - data.getUint32(20) === expected.rows &&
    data.getUint16(40) === 1 && (data.getUint8(42) & 127) + 1 === expected.bits &&
    Boolean(data.getUint8(42) & 128) === expected.signed && data.getUint8(43) === 1 && data.getUint8(44) === 1;
...
export function decodeFrame(library, encoded, expected) {
  ...
  try {
    ...
    if (info.width !== expected.columns || info.height !== expected.rows || info.componentCount !== 1 || info.bitsPerSample !== expected.bits || Boolean(info.isSigned) !== expected.signed) throw new Error('Decoded JPEG 2000 pixels do not match the DICOM metadata.');
    const copy = decoder.getDecodedBuffer().slice();
    return expected.bits > 8 ? new Uint16Array(copy.buffer, copy.byteOffset, copy.byteLength / 2) : copy;
